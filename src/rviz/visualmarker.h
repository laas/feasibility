...
namespace ros{
  ...
  class Geometry{
    double rx_,ry_,rz_,rw_; //quaternions
  public:
    Geometry();
    double x_,y_,z_;
  ...
  struct Color{
    ...
    double r_,g_,b_,a_;
    void print(){
      ROS_INFO("COLOR -> %f %f %f %f", r_,g_,b_,a_);
    }
  };
  ...
  protected:
    static RVIZInterface *rviz_; 
    static uint global_id_;
    uint id_;
    Color c_;

    visualization_msgs::Marker marker_;
    visualization_msgs::InteractiveMarker active_marker_;
    static boost::shared_ptr<interactive_markers::InteractiveMarkerServer> server_;

    std::string text_;
    bool textHover;
    std::string geometry_subscribe_topic_;

    boost::shared_ptr<boost::thread> m_thread;
    boost::shared_ptr<boost::thread> m_thread_interactive_marker;

    ros::Subscriber m_subscriber;
    bool changedPosition_;
  ...
  private:
    ...
    Geometry g_old_;
    double const_offset_x_, const_offset_y_; //additional offset for evart simulation
    double const_offset_yaw_;
